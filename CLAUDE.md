...
- **Update the manifest.** Adding an image means adding both
  `images/<name>/` *and* an entry under `[images]` in
  [`loadsmith-lab.toml`](loadsmith-lab.toml) — `loadsmith-lab list`/`build` and
  `origin show images` read from the manifest, not the filesystem.
